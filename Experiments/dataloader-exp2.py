...
class Dataset(InMemoryDataset):
    ...
    def read_data(self):
        ...
        # 读取ratings文件
        ratings_df = pd.read_csv(self.ratingfile, sep=self.sep, header=None, names=['user_id', 'item_id', 'rating'])

        # 过滤掉评分列中只有1个样本的评分（如果某个评分值的样本数小于2）
        class_counts = ratings_df['rating'].value_counts()  # 获取评分值的数量
        valid_ratings = class_counts[class_counts > 1].index  # 保留评分数大于1的评分值
        ratings_df = ratings_df[ratings_df['rating'].isin(valid_ratings)]  # 只保留这些评分的数据

        # 使用分层抽样进行数据拆分
        train_df, test_df = train_test_split(ratings_df, test_size=0.4, random_state=self.args.random_seed, stratify=ratings_df['rating'])
        test_df, valid_df = train_test_split(test_df, test_size=0.5, random_state=self.args.random_seed, stratify=test_df['rating'])
